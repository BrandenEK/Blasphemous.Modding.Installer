...
internal class Skin
{
    ...
    public bool Installed => File.Exists(Path.Combine(PathToSkinFolder, TempGetInfoFileName(_skinType)));

    public Version LocalVersion
    {
        get
        {
            string infoPath = Path.Combine(PathToSkinFolder, TempGetInfoFileName(_skinType));
            if (Installed)
            {
    ...
    public string PathToSkinFolder => _skinType == SectionType.Blas1Skins
        ? $"{RootFolder}/Modding/skins/{Data.id}"
        : $"{RootFolder}/Modding/custom_skins/{Data.id}";
    ...
    public string InfoURL => $"https://raw.githubusercontent.com/BrandenEK/Blasphemous.Community.Skins/main/{GithubSubFolder}/{Data.id}/{TempGetInfoFileName(_skinType)}";
    ...
    public async void Install()
    {
        ...
        // Another hacky solution where now the blas2 method is gone, and it just switches info.txt with info.json depending on the skin type
        InstallBlas1Skin();
    ...
    private async void InstallBlas1Skin()
    {
        ...
        bool infoExists = ExistsInCache(TempGetInfoFileName(_skinType), out string infoCache);
        ...
        File.Copy(infoCache, Path.Combine(installPath, TempGetInfoFileName(_skinType)));
        File.Copy(textureCache, Path.Combine(installPath, "texture.png"));
